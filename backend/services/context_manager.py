...
async def create_prompt(user_supabase, user_id: str, chat_id: str, user_message: str) -> str:
    try:
        ...
        pref_resp = user_supabase.table("profiles").select("theme, language, timezone, communication_style, response_length, expertise_level, default_model, temperature, max_tokens").eq("id", user_id).execute()
