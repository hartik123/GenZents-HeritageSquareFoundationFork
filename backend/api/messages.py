...
from models.message import MessageCreate, MessageResponse, MessageUpdate, TestRequest, TestResponse
...
from services.drive_agent import GoogleDriveAgent
...
@router.post("/test/chat", response_model=TestResponse)
async def test_messages(request: TestRequest):
    drive_agent = GoogleDriveAgent()
    drive_agent._create_model()
    logger.info("drive agent set up")
    response = drive_agent.process_message(request.content)
    logger.info(response)
    return TestResponse(content=response) 

@router.get("/test/store_all_folders")
async def store_folder_handler():
    drive_agent = GoogleDriveAgent()
    response = drive_agent.store_all_folders()
    return TestResponse(content="Done")